...
const TOOL = path.join(__dirname, "..", "tools", "approval-recipient-count.js");
const CHILD_TIMEOUT_MS = 5_000;

const USAGE_ERROR =
  "usage: node app/tools/approval-recipient-count.js <state-file> <recipient>\n";
const READ_ERROR = "approval-recipient-count: cannot read state file\n";
const PARSE_ERROR = "approval-recipient-count: state file is not valid JSON\n";
const INVALID_STATE_ERROR = "approval-recipient-count: invalid Relay state\n";
...
const COUNT_TWO_STDOUT = '{"approvalsForRecipient":2}\n';
const COUNT_ONE_STDOUT = '{"approvalsForRecipient":1}\n';
const COUNT_ZERO_STDOUT = '{"approvalsForRecipient":0}\n';
...
const STORED_ACTION_TO = FIXTURE_A_VALUE.approvals.map(approval => approval.action.to);
...
const EXPECTED_SENSITIVE_TOKENS = [
  ...
  "APPROVAL-ID-TARGET-ONE-MUST-NOT-LEAK",
  "JOB-ID-TARGET-ONE-MUST-NOT-LEAK",
  "TARGET-RECIPIENT-MUST-NOT-LEAK@example.test",
  "SUBJECT-TARGET-ONE-MUST-NOT-LEAK",
  "BODY-TARGET-ONE-MUST-NOT-LEAK",
  "APPROVAL-ID-OTHER-MUST-NOT-LEAK",
  "JOB-ID-OTHER-MUST-NOT-LEAK",
  "OTHER-RECIPIENT-MUST-NOT-LEAK@example.test",
  "SUBJECT-OTHER-MUST-NOT-LEAK",
  "BODY-OTHER-MUST-NOT-LEAK",
  "APPROVAL-ID-NUMERIC-MUST-NOT-LEAK",
  "JOB-ID-NUMERIC-MUST-NOT-LEAK",
  "IGNORED-ACTION-FIELD-MUST-NOT-LEAK",
  "IGNORED-APPROVAL-FIELD-MUST-NOT-LEAK",
  "APPROVAL-ID-TARGET-TWO-MUST-NOT-LEAK",
  "JOB-ID-TARGET-TWO-MUST-NOT-LEAK",
  "SUBJECT-TARGET-TWO-MUST-NOT-LEAK",
  "BODY-TARGET-TWO-MUST-NOT-LEAK",
  "REJECTION-REASON-MUST-NOT-LEAK",
  "APPROVAL-ID-LEADING-MUST-NOT-LEAK",
  "JOB-ID-LEADING-MUST-NOT-LEAK",
  " TARGET-RECIPIENT-MUST-NOT-LEAK@example.test",
  "SUBJECT-LEADING-MUST-NOT-LEAK",
  "BODY-LEADING-MUST-NOT-LEAK",
  "APPROVAL-ID-UNICODE-MUST-NOT-LEAK",
  "JOB-ID-UNICODE-MUST-NOT-LEAK",
  "SUBJECT-UNICODE-MUST-NOT-LEAK",
  "BODY-UNICODE-MUST-NOT-LEAK",
...
function makeTempDir(label) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `relay-approval-recipient-count-${label}-`));
}
...
test("AC1: two invocations for the repeated target recipient each print the frozen count of two and leave Fixture A untouched", () => {
  ...
  try {
    ...
    for (const attempt of [1, 2]) {
      const result = runTool([fixture, TARGET_RECIPIENT]);
...
test("AC2: the other, leading-space, and composed Unicode recipients each print the frozen count of one", () => {
  const queries = [
    ["other-recipient", OTHER_RECIPIENT],
    ["leading-space-recipient", LEADING_SPACE_RECIPIENT],
    ["composed-unicode-recipient", COMPOSED_RECIPIENT],
  ];

  for (const [label, recipient] of queries) {
    const matches = STORED_ACTION_TO.filter(storedTo => storedTo === recipient);
    assert.strictEqual(
      matches.length,
      1,
      `${label} is strictly equal to exactly one stored action.to`
    );
  }
  ...
  try {
    ...
    for (const [label, recipient] of queries) {
      const result = runTool([fixture, recipient]);
...
test("AC3: the five frozen zero-match queries succeed with the frozen count of zero", () => {
  assert.strictEqual(
    FIXTURE_A_VALUE.approvals[2].action.to,
    7,
    "Fixture A's third approval stores the JSON number 7 as action.to"
  );
  assert.strictEqual(
    typeof FIXTURE_A_VALUE.approvals[2].action.to,
    "number",
    "Fixture A's third stored action.to is numeric, not a string"
  );
  ...
  assert.strictEqual(EMPTY_RECIPIENT, "", "the empty query is the exact zero-byte string");
  assert.strictEqual(EMPTY_RECIPIENT.length, 0, "the empty query carries zero bytes");

  for (const storedTo of STORED_ACTION_TO) {
    assert.notStrictEqual(storedTo, "", "no stored action.to is the empty string");
    if (typeof storedTo === "string") {
      assert.notStrictEqual(
        Buffer.from(storedTo, "utf8").toString("hex"),
        Buffer.from(CASE_DISTINCT_RECIPIENT, "utf8").toString("hex"),
        "the case-distinct query is byte-distinct from every stored string action.to"
      );
    }
  }
  ...
  for (const [label, contents, recipient] of [
    ["case-distinct-recipient", FIXTURE_A, CASE_DISTINCT_RECIPIENT],
    ["numeric-string-recipient", FIXTURE_A, NUMERIC_RECIPIENT],
    ["empty-recipient", FIXTURE_A, EMPTY_RECIPIENT],
    ["decomposed-unicode-recipient", FIXTURE_A, DECOMPOSED_RECIPIENT],
    ["fixture-b-empty-approvals", FIXTURE_B, TARGET_RECIPIENT],
  ]) {
  ...
  for (const label of ["zero", "one", "three"]) {
    ...
    try {
      ...
      if (label === "zero") {
        ...
        args = [candidate, TARGET_RECIPIENT, "extra"];
      }
...
test("AC5: an absent path and a directory path fail with the exact read-failure envelope and change nothing", () => {
  ...
  try {
    ...
    const result = runTool([missing, TARGET_RECIPIENT]);
  ...
  try {
    ...
    const result = runTool([stateDirectory, TARGET_RECIPIENT]);
...
test("AC7: every S1-S30 structural-invalid fixture fails with the exact invalid-state envelope after a distinctness proof", () => {
  ...
  assert.strictEqual(
    ...
    "pending",
    "S30's first approval is a complete valid approval"
  );
  assert.strictEqual(
