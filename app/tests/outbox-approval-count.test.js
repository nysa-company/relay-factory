...
const TOOL = path.join(__dirname, "..", "tools", "outbox-approval-count.js");
const CHILD_TIMEOUT_MS = 5_000;

const USAGE_ERROR =
  "usage: node app/tools/outbox-approval-count.js <state-file> <approval-id>\n";
const READ_ERROR = "outbox-approval-count: cannot read state file\n";
const PARSE_ERROR = "outbox-approval-count: state file is not valid JSON\n";
const INVALID_STATE_ERROR = "outbox-approval-count: invalid Relay state\n";
...
const LEADING_SPACE_APPROVAL = " TARGET-APPROVAL-MUST-NOT-LEAK";
const EMPTY_APPROVAL = "";

const COUNT_ONE_STDOUT = '{"outboxEntries":1}\n';
const COUNT_ZERO_STDOUT = '{"outboxEntries":0}\n';
...
const STRUCTURAL_INVALID_FIXTURES = [
  ...
  [
    ...
    '{"events":[],"jobs":[],"approvals":[],"outbox":[{"to":"a","subject":"b","body":"c",' +
      ...
      '{"to":"d","subject":"e","body":"f",' +
      '"approvalId":"TARGET-APPROVAL-MUST-NOT-LEAK","sentAt":"time-b"}]}\n',
  ],
  [
    "S20",
    '{"events":[],"jobs":[],"approvals":[],"outbox":[{"to":"a","subject":"b","body":"c",' +
...
function makeTempDir(label) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `relay-outbox-approval-count-${label}-`));
}
...
test("AC1: two invocations for the target approval ID each print the frozen count of one and leave Fixture A untouched", () => {
  ...
  try {
    ...
    for (const attempt of [1, 2]) {
      const result = runTool([fixture, TARGET_APPROVAL]);
      ...
      assert.strictEqual(result.stdout, COUNT_ONE_STDOUT, `stdout on invocation ${attempt}`);
...
test("AC2: the other stored approval ID in first position prints the frozen count of one and leaves Fixture A untouched", () => {
  ...
  try {
    ...
    const result = runTool([fixture, OTHER_APPROVAL]);

    assert.strictEqual(result.stderr, "", "stderr for the other approval ID");
    assert.strictEqual(result.stderrBytes, 0, "stderr byte count for the other approval ID");
    assert.strictEqual(result.stdout, COUNT_ONE_STDOUT, "stdout for the other approval ID");
    assert.strictEqual(result.status, 0, "exit code for the other approval ID");
    assert.deepStrictEqual(snapshotFile(fixture), before, "fixture bytes and bigint stat");
...
test("AC3: all four frozen no-match cases are successful zero-count queries", () => {
  const storedApprovalIds = FIXTURE_A_VALUE.outbox.map(receipt => receipt.approvalId);
  for (const literal of [CASE_DISTINCT_APPROVAL, LEADING_SPACE_APPROVAL, EMPTY_APPROVAL]) {
    for (const storedApprovalId of storedApprovalIds) {
      assert.notStrictEqual(
        storedApprovalId,
        literal,
        `no-match literal ${JSON.stringify(literal)} is byte-distinct from every stored approvalId`
      );
    }
  }
  assert.strictEqual(EMPTY_APPROVAL.length, 0, "the empty query carries zero bytes");
  ...
  for (const [label, contents, approvalId] of [
    ["case-distinct-approval-id", FIXTURE_A, CASE_DISTINCT_APPROVAL],
    ["leading-space-approval-id", FIXTURE_A, LEADING_SPACE_APPROVAL],
    ["empty-approval-id", FIXTURE_A, EMPTY_APPROVAL],
    ["fixture-b-empty-outbox", FIXTURE_B, TARGET_APPROVAL],
  ]) {
  ...
  for (const label of ["zero", "one", "three"]) {
    ...
    try {
      ...
      if (label === "zero") {
        ...
        args = [candidate, TARGET_APPROVAL, "extra"];
      }
...
test("AC5: an absent path and a directory path fail with the exact read-failure envelope and change nothing", () => {
  ...
  try {
    ...
    const result = runTool([missing, TARGET_APPROVAL]);
  ...
  try {
    ...
    const result = runTool([stateDirectory, TARGET_APPROVAL]);
...
test("AC7: every S1-S20 structural-invalid fixture fails with the exact invalid-state envelope after a distinctness proof", () => {
  const serialized = [
    ["A", FIXTURE_A],
    ["B", FIXTURE_B],
    ["C", FIXTURE_C],
  ...
  const emptyQueryDir = makeTempDir("ac7-s20-empty-query");
  try {
    const fixture = writeFixture(
      emptyQueryDir,
      "state.json",
      STRUCTURAL_INVALID_FIXTURES[S20_INDEX][1]
    );
    const before = snapshotFile(fixture);
    const parentBefore = entryNames(emptyQueryDir);

    const result = runTool([fixture, EMPTY_APPROVAL]);

    assert.strictEqual(result.stdout, "", "stdout for S20 with the empty query");
    assert.strictEqual(result.stdoutBytes, 0, "stdout byte count for S20 with the empty query");
    assert.strictEqual(result.stderr, INVALID_STATE_ERROR, "stderr for S20 with the empty query");
    assert.strictEqual(result.status, 1, "exit code for S20 with the empty query");
    assert.deepStrictEqual(
      snapshotFile(fixture),
      before,
      "S20 bytes and stat after the empty query"
    );
    assert.deepStrictEqual(
      entryNames(emptyQueryDir),
      parentBefore,
      "parent entries after the empty query"
    );
  } finally {
    fs.rmSync(emptyQueryDir, { recursive: true, force: true });
  }
});
...
test("AC8: the command self-terminates under the frozen timeout and both files stay inside their frozen source boundaries", () => {
  ...
  assert.strictEqual(fs.existsSync(TOOL), true, "app/tools/outbox-approval-count.js exists");
