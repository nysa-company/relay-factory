...
const TOOL = path.join(__dirname, "..", "tools", "outbox-recipient-count.js");
const CHILD_TIMEOUT_MS = 5_000;

const USAGE_ERROR =
  "usage: node app/tools/outbox-recipient-count.js <state-file> <recipient>\n";
const READ_ERROR = "outbox-recipient-count: cannot read state file\n";
const PARSE_ERROR = "outbox-recipient-count: state file is not valid JSON\n";
const INVALID_STATE_ERROR = "outbox-recipient-count: invalid Relay state\n";
...
const COUNT_TWO_STDOUT = '{"outboxEntries":2}\n';
...
const FIXTURE_B =
  '{"events":[null],"jobs":[7],"approvals":["OPAQUE-APPROVAL-MUST-NOT-LEAK"],' +
...
const STRUCTURAL_INVALID_FIXTURES = [
  ...
  [
    ...
    '{"events":[],"jobs":[],"approvals":[],"outbox":[' +
      '{"to":"TARGET-RECIPIENT-MUST-NOT-LEAK@example.test","subject":"b","body":"c",' +
      '"approvalId":"appr-valid-first","sentAt":"time-a"},{"to":"d","subject":"e","body":"f",' +
      '"approvalId":"appr-invalid-second"}]}\n',
  ],
];
...
function makeTempDir(label) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `relay-outbox-recipient-count-${label}-`));
}
...
test("AC2: the other stored recipient prints the frozen count of one and leaves Fixture A untouched", () => {
  ...
  try {
    ...
    const result = runTool([fixture, OTHER_RECIPIENT]);

    assert.strictEqual(result.stderr, "", "stderr for the other recipient");
    assert.strictEqual(result.stderrBytes, 0, "stderr byte count for the other recipient");
    assert.strictEqual(result.stdout, COUNT_ONE_STDOUT, "stdout for the other recipient");
    assert.strictEqual(result.status, 0, "exit code for the other recipient");
...
test("AC3: the case-distinct recipient and Fixture B's empty outbox are successful zero-count queries", () => {
  for (const storedTo of FIXTURE_A_VALUE.outbox.map(receipt => receipt.to)) {
    assert.notStrictEqual(
      storedTo,
      CASE_DISTINCT_RECIPIENT,
      "the case-distinct query literal is byte-distinct from every stored to value"
    );
  }
  ...
  for (const [label, contents, recipient] of [
    ...
    ["fixture-b-empty-outbox", FIXTURE_B, TARGET_RECIPIENT],
  ]) {
...
test("AC8: the command self-terminates under the frozen timeout and both files stay inside their frozen source boundaries", () => {
  ...
  assert.strictEqual(fs.existsSync(TOOL), true, "app/tools/outbox-recipient-count.js exists");
...
test("AC10: a numeric-looking recipient and the empty recipient are strict-equality zero-count queries", () => {
  assert.strictEqual(
    FIXTURE_A_VALUE.outbox[2].to,
    7,
    "Fixture A's third outbox element stores the JSON number 7"
  );
  assert.strictEqual(
    typeof FIXTURE_A_VALUE.outbox[2].to,
    "number",
    "Fixture A's third stored to value is numeric, not a string"
  );
  ...
  for (const storedTo of FIXTURE_A_VALUE.outbox.map(receipt => receipt.to)) {
    assert.notStrictEqual(storedTo, "", "no stored to value is the empty string");
  }
  ...
  try {
    ...
    for (const [label, recipient] of [
      ["numeric-string-recipient", NUMERIC_RECIPIENT],
      ["empty-recipient", EMPTY_RECIPIENT],
    ]) {
      const result = runTool([fixture, recipient]);
