...
const TOOL = path.join(__dirname, "..", "tools", "job-count.js");
const CHILD_TIMEOUT_MS = 5_000;

const USAGE_ERROR = "usage: node app/tools/job-count.js <state-file>\n";
const READ_ERROR = "job-count: cannot read state file\n";
const PARSE_ERROR = "job-count: state file is not valid JSON\n";
const INVALID_STATE_ERROR = "job-count: invalid Relay state\n";

const FIXTURE_A_STDOUT = '{"jobs":3}\n';
const FIXTURE_B_STDOUT = '{"jobs":0}\n';
...
const STRUCTURAL_INVALID_FIXTURES = [
  ...
  [
    "S25",
    '{"events":[],"jobs":[{"id":"job-valid-first","eventId":"event-valid","status":"done",' +
      '"attempts":1,"lastError":null,"retries":0,"attemptsSinceRetry":1},' +
...
function makeTempDir(label) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `relay-job-count-${label}-`));
}
...
test("AC1: two invocations against Fixture A each print the frozen three-job count and leave the fixture untouched", () => {
...
test("AC6: every S1-S25 structural-invalid fixture fails with the exact invalid-state envelope after a distinctness proof", () => {
  ...
  assert.strictEqual(s25.jobs[0].status, "done", "S25's first job is a valid job");
  assert.strictEqual(
...
test("AC7: the production command self-terminates under the frozen timeout and both files stay inside their frozen source boundaries", () => {
  ...
  assert.strictEqual(fs.existsSync(TOOL), true, "app/tools/job-count.js exists");
...
test("field independence: a pending job with prior attempts and a string lastError is counted as valid", () => {
  ...
  try {
      ...
      JSON.stringify(
        {
          ...
          jobs: [
            {
              id: "job-pending-retrying",
              eventId: "event-pending-retrying",
              status: "pending",
              attempts: 2,
    ...
    assert.strictEqual(result.stderr, "", "stderr for the mid-retry pending job");
    assert.strictEqual(result.stdout, '{"jobs":1}\n', "stdout for the mid-retry pending job");
    assert.strictEqual(result.status, 0, "exit code for the mid-retry pending job");
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
