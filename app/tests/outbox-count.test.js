...
const TOOL = path.join(__dirname, "..", "tools", "outbox-count.js");
const CHILD_TIMEOUT_MS = 5_000;

const USAGE_ERROR = "usage: node app/tools/outbox-count.js <state-file>\n";
const READ_ERROR = "outbox-count: cannot read state file\n";
const PARSE_ERROR = "outbox-count: state file is not valid JSON\n";
const INVALID_STATE_ERROR = "outbox-count: invalid Relay state\n";

const FIXTURE_A_STDOUT = '{"outboxEntries":3}\n';
const FIXTURE_B_STDOUT = '{"outboxEntries":0}\n';
...
const SENSITIVE_TOKENS = [
  ...
  "RECIPIENT-CHARLIE-MUST-NOT-LEAK",
...
function makeTempDir(label) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `relay-outbox-count-${label}-`));
}
...
test("AC1: two invocations against Fixture A each print the frozen three-entry count and leave the fixture untouched", () => {
...
test("AC6: every S1-S20 structural-invalid fixture fails with the exact invalid-state envelope after a distinctness proof", () => {
  const serialized = [
    ["A", FIXTURE_A],
    ["B", FIXTURE_B],
    ["C", FIXTURE_C],
...
test("AC7: the production command self-terminates under the frozen timeout and both files stay inside their frozen source boundaries", () => {
  ...
  assert.strictEqual(fs.existsSync(TOOL), true, "app/tools/outbox-count.js exists");
...
test("arity permutation: three positional arguments fail with the exact usage envelope", () => {
  ...
  try {
    ...
    const result = runTool([candidate, "extra", "surplus"]);
    ...
    assert.deepStrictEqual(entryNames(dir), [], "parent stayed empty");
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test("field independence: distinct receipts with opaque message values and unresolved approval IDs are counted", () => {
  ...
  try {
          ...
          outbox: [
            {
              ...
              subject: false,
              body: 0,
              approvalId: "appr-unresolved-first",
              sentAt: "not-an-iso-instant",
            },
            {
              to: "operator@example.test",
              subject: "subject",
              body: "body",
              approvalId: "appr-unresolved-second",
              sentAt: "2026-08-24T12:00:00.000Z",
            },
          ],
        },
    ...
    assert.strictEqual(result.stderr, "", "stderr for the field-independence fixture");
    assert.strictEqual(
      result.stdout,
      '{"outboxEntries":2}\n',
      "stdout for the field-independence fixture"
    );
    assert.strictEqual(result.status, 0, "exit code for the field-independence fixture");
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
