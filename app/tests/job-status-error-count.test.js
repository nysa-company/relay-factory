...
test("AC9: child calls have the frozen timeout and production and test source boundaries", () => {
  ...
  assert.strictEqual(fs.existsSync(TOOL), true); const source = fs.readFileSync(TOOL, "utf8"); assert.deepStrictEqual(specifiers(source), ["node:fs"]); assert.deepStrictEqual([...source.matchAll(/\bfs\s*\.\s*([A-Za-z_$][\w$]*)/g)].map(match => match[1]).filter((value, index, values) => values.indexOf(value) === index).sort(), ["readFileSync"]); for (const forbidden of ["server.js", "fetch(", "WebSocket", "node:http", "node:https", "node:net", "node:child_process", "setTimeout", "setInterval"]) assert.strictEqual(source.includes(forbidden), false); assert.deepStrictEqual(specifiers(fs.readFileSync(__filename, "utf8")), ["node:assert", "node:child_process", "node:fs", "node:os", "node:path", "node:test"]);
});

test("AC10: tests-first history preserves the two owned paths and post-implementation verification commands", () => {
  const history = spawnSync("git", ["log", "--format=%H%x00%s", "--all"], { cwd: path.join(__dirname, "..", ".."), encoding: "utf8" }); assert.strictEqual(history.status, 0); const rows = history.stdout.split("\n").filter(Boolean).map(row => row.split("\0")); const testCommit = rows.find(([, subject]) => subject.includes("T-279") && subject.toLowerCase().includes("test")); const builderCommit = rows.find(([, subject]) => subject.includes("T-279") && subject.toLowerCase().includes("build")); assert.notStrictEqual(testCommit, undefined); assert.notStrictEqual(builderCommit, undefined); const testPosition = rows.indexOf(testCommit); const builderPosition = rows.indexOf(builderCommit); assert.ok(testPosition > builderPosition); for (const [commit, expected] of [[testCommit[0], "app/tests/job-status-error-count.test.js"], [builderCommit[0], "app/tools/job-status-error-count.js"]]) { const changed = spawnSync("git", ["diff-tree", "--no-commit-id", "--name-only", "-r", commit], { cwd: path.join(__dirname, "..", ".."), encoding: "utf8" }); assert.strictEqual(changed.status, 0); assert.deepStrictEqual(changed.stdout.trim().split("\n").filter(name => !name.startsWith("factory/tickets/")), [expected]); }
});
