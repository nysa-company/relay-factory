...
const TOOL = path.join(__dirname, "..", "tools", "event-metadata.js");
const CHILD_TIMEOUT_MS = 5_000;

const USAGE_ERROR = "usage: node app/tools/event-metadata.js <state-file> <event-id>\n";
const READ_ERROR = "event-metadata: cannot read state file\n";
const PARSE_ERROR = "event-metadata: state file is not valid JSON\n";
const INVALID_STATE_ERROR = "event-metadata: invalid Relay state\n";
const NOT_FOUND_ERROR = "event-metadata: event not found\n";
...
const STRUCTURAL_INVALID_FIXTURES = [
  ...
  [
    "S10",
    '{"events":[{"type":"meeting","receivedAt":"2026-08-07T12:00:00.000Z"}],"jobs":[],"approvals":[],"outbox":[]}\n',
  ],
  [
    "S11",
  ...
  [
    "S13",
    '{"events":[{"id":"event-alpha","receivedAt":"2026-08-07T12:00:00.000Z"}],"jobs":[],"approvals":[],"outbox":[]}\n',
  ],
  [
    "S14",
    '{"events":[{"id":"event-alpha","type":"","receivedAt":"2026-08-07T12:00:00.000Z"}],"jobs":[],"approvals":[],"outbox":[]}\n',
  ],
  [
    "S15",
    '{"events":[{"id":"event-alpha","type":7,"receivedAt":"2026-08-07T12:00:00.000Z"}],"jobs":[],"approvals":[],"outbox":[]}\n',
  ],
  ["S16", '{"events":[{"id":"event-alpha","type":"meeting"}],"jobs":[],"approvals":[],"outbox":[]}\n'],
  [
    "S17",
  ...
  [
    "S19",
    '{"events":[{"id":"event-alpha","type":"meeting","receivedAt":"2026-08-07T12:00:00.000Z"},' +
      '{"id":"event-alpha","type":"email","receivedAt":"2026-08-07T12:00:01.000Z"}],' +
      '"jobs":[],"approvals":[],"outbox":[]}\n',
  ],
];

function makeTempDir(label) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `relay-event-metadata-${label}-`));
}
...
test("AC1: repeated event-alpha lookups against Fixture A print the frozen projection and leave the fixture untouched", () => {
...
test("AC2: the event-zulu lookup against Fixture A prints its own frozen projection and leaves the fixture untouched", () => {
...
test("AC4: zero, one, and three positional arguments fail with the exact usage envelope and read nothing", () => {
...
test("AC5: an absent path and a directory path fail with the exact read-failure envelope and change nothing", () => {
  ...
  try {
    const missing = path.join(missingDir, "missing-state.json");
...
test("AC6: Fixture C fails with the exact parse-failure envelope and never echoes the malformed bytes", () => {
  ...
  try {
    ...
    assert.strictEqual(result.stdout.includes("MALFORMED-PAYLOAD-MUST-NOT-LEAK"), false);
    assert.strictEqual(result.stderr.includes("MALFORMED-PAYLOAD-MUST-NOT-LEAK"), false);
    assert.deepStrictEqual(snapshotFile(fixture), before);
    assert.deepStrictEqual(entryNames(dir), parentBefore);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
test("AC8: the production command self-terminates under the frozen timeout and stays inside its offline source boundary", () => {
  ...
  assert.strictEqual(fs.existsSync(TOOL), true, "app/tools/event-metadata.js exists");
