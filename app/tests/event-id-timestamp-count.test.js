...
const TOOL = path.join(__dirname, "..", "tools", "event-id-timestamp-count.js");
const CHILD_TIMEOUT_MS = 5_000;

const USAGE_ERROR =
  "usage: node app/tools/event-id-timestamp-count.js <state-file> <event-id> <timestamp>\n";
const READ_ERROR = "event-id-timestamp-count: cannot read state file\n";
const PARSE_ERROR = "event-id-timestamp-count: state file is not valid JSON\n";
const INVALID_STATE_ERROR = "event-id-timestamp-count: invalid Relay state\n";

const ONE_STDOUT = '{"eventsWithIdAndTimestamp":1}\n';
const ZERO_STDOUT = '{"eventsWithIdAndTimestamp":0}\n';

const CANONICAL_ID = "event-alpha";
const CANONICAL_TS = "2026-08-25T12:00:00.000Z";

// Escape sequences keep the frozen byte forms stable against editor
// Unicode normalization: composed é is U+00E9, decomposed is U+0065 U+0301.
const COMPOSED_CAFE_ID = "event-caf\u00E9";
const DECOMPOSED_CAFE_ID = "event-cafe\u0301";
const COMPOSED_CAFE_TS = "instant-caf\u00E9";
const DECOMPOSED_CAFE_TS = "instant-cafe\u0301";
...
const POSITIVE_PAIRS = [
  ["event-bravo", "2026-08-25T12:00:01.000Z"],
  ["Event-alpha", "2026-08-25T07:00:00-05:00"],
  [" event-alpha", " 2026-08-25T12:00:00.000Z"],
  ["007", "7"],
  [COMPOSED_CAFE_ID, COMPOSED_CAFE_TS],
  ["event-verbatim", "VERBATIM-TIMESTAMP-MUST-NOT-BE-NORMALIZED"],
];

const ZERO_PAIRS = [
  ["event-alpha", "2026-08-25T12:00:01.000Z"],
  ["Event-alpha", "2026-08-25T12:00:00.000Z"],
  ["event-alpha", "2026-08-25T07:00:00-05:00"],
  [" event-alpha", "2026-08-25T12:00:00.000Z"],
  ["event-alpha", " 2026-08-25T12:00:00.000Z"],
  ["7", "7"],
  [DECOMPOSED_CAFE_ID, COMPOSED_CAFE_TS],
  [COMPOSED_CAFE_ID, DECOMPOSED_CAFE_TS],
  ["event-verbatim", "verbatim-TIMESTAMP-MUST-NOT-BE-NORMALIZED"],
];
...
const STRUCTURAL_INVALID_FIXTURES = [
  ...
  [
    "S20",
    '{"events":[{"id":"event-alpha","type":"meeting","receivedAt":"2026-08-25T12:00:00.000Z"},' +
      '{"id":"event-invalid-second","receivedAt":"2026-08-25T12:00:01.000Z"}],' +
      '"jobs":[],"approvals":[],"outbox":[]}\n',
  ],
];

function makeTempDir(label) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `relay-event-id-ts-count-${label}-`));
}
...
function runTool(args) {
  assert.strictEqual(
    fs.existsSync(TOOL),
    true,
    "app/tools/event-id-timestamp-count.js exists"
  );
...
function matchingEventCount(id, timestamp) {
  return FIXTURE_A_VALUE.events.filter(
    event => event.id === id && event.receivedAt === timestamp
  ).length;
}
...
test("AC1: two canonical-pair invocations against Fixture A each print the frozen single-match count and leave the fixture untouched", () => {
  ...
  try {
    ...
    for (const attempt of [1, 2]) {
      const result = runTool([fixture, CANONICAL_ID, CANONICAL_TS]);
...
test("AC2: every frozen positive filter pair strictly matches exactly one stored event and prints the frozen single-match count", () => {
  for (const [id, timestamp] of POSITIVE_PAIRS) {
    assert.strictEqual(
      matchingEventCount(id, timestamp),
      1,
      `pair ${JSON.stringify([id, timestamp])} strictly equals exactly one stored event`
    );
  }
  ...
  try {
    ...
    for (const [id, timestamp] of POSITIVE_PAIRS) {
      const label = JSON.stringify([id, timestamp]);
      const result = runTool([fixture, id, timestamp]);
...
test("AC3: every frozen zero-match pair proves its byte distinction, matches no stored event, and prints the frozen zero count", () => {
  assert.notStrictEqual("event-alpha", "Event-alpha", "case pair is byte-distinct");
  assert.notStrictEqual(" event-alpha", "event-alpha", "leading-space ID pair is byte-distinct");
  assert.strictEqual(" event-alpha".charCodeAt(0), 0x20, "leading-space ID starts with U+0020");
  assert.notStrictEqual(
    " 2026-08-25T12:00:00.000Z",
    "2026-08-25T12:00:00.000Z",
    "leading-space timestamp pair is byte-distinct"
  );
  assert.strictEqual(
    " 2026-08-25T12:00:00.000Z".charCodeAt(0),
    0x20,
    "leading-space timestamp starts with U+0020"
  );
  assert.notStrictEqual("007", "7", "leading-zero pair is byte-distinct");
  assert.notStrictEqual(
    "2026-08-25T12:00:00.000Z",
    "2026-08-25T07:00:00-05:00",
    "timezone-representation pair is byte-distinct"
  );
  assert.strictEqual(
    Date.parse("2026-08-25T12:00:00.000Z"),
    Date.parse("2026-08-25T07:00:00-05:00"),
    "canonical and offset strings denote the same instant"
  );
  assert.notStrictEqual(
    "VERBATIM-TIMESTAMP-MUST-NOT-BE-NORMALIZED",
    "verbatim-TIMESTAMP-MUST-NOT-BE-NORMALIZED",
    "timestamp-case pair is byte-distinct"
  );
  assert.notStrictEqual(COMPOSED_CAFE_ID, DECOMPOSED_CAFE_ID, "Unicode ID pair is byte-distinct");
  assert.notStrictEqual(
    COMPOSED_CAFE_TS,
    DECOMPOSED_CAFE_TS,
    "Unicode timestamp pair is byte-distinct"
  );
  assert.strictEqual(
    Buffer.from("caf\u00E9", "utf8").toString("hex"),
    "636166c3a9",
    "composed café has the frozen UTF-8 bytes"
  );
  assert.strictEqual(
    Buffer.from("cafe\u0301", "utf8").toString("hex"),
    "63616665cc81",
    "decomposed café has the frozen UTF-8 bytes"
  );

  for (const [id, timestamp] of ZERO_PAIRS) {
    assert.strictEqual(
      matchingEventCount(id, timestamp),
      0,
      `pair ${JSON.stringify([id, timestamp])} matches no stored event`
    );
  }

  const dir = makeTempDir("ac3-a");
  try {
    const fixture = writeFixture(dir, "state.json", FIXTURE_A);
    const before = snapshotFile(fixture);
    const parentBefore = entryNames(dir);
  ...
  const dirB = makeTempDir("ac3-b");
  try {
    const fixture = writeFixture(dirB, "state.json", FIXTURE_B);
    const before = snapshotFile(fixture);
    const parentBefore = entryNames(dirB);
    ...
    assert.strictEqual(result.stdout, ZERO_STDOUT, "stdout for Fixture B");
    ...
    assert.deepStrictEqual(entryNames(dirB), parentBefore, "parent entries for Fixture B");
  } finally {
...
test("AC4: wrong arity and empty filters fail with the exact usage envelope before any file access", () => {
  const vectors = [
    ...
    ["two-arguments", candidate => [candidate, CANONICAL_ID]],
    ["four-arguments", candidate => [candidate, CANONICAL_ID, CANONICAL_TS, "extra"]],
    ["empty-event-id", candidate => [candidate, "", CANONICAL_TS]],
    ["empty-timestamp", candidate => [candidate, CANONICAL_ID, ""]],
  ];

  for (const [label, buildArgs] of vectors) {
...
test("AC5: an absent path and a directory path fail with the exact read-failure envelope and change nothing", () => {
  ...
  try {
    ...
    const result = runTool([missing, CANONICAL_ID, CANONICAL_TS]);
  ...
  try {
    ...
    const result = runTool([stateDirectory, CANONICAL_ID, CANONICAL_TS]);
...
test("AC7: every S1-S20 structural-invalid fixture fails with the exact invalid-state envelope after distinctness and precedence proofs", () => {
  const serialized = [
    ["A", FIXTURE_A],
    ["B", FIXTURE_B],
    ["C", FIXTURE_C],
  ...
  assert.strictEqual(STRUCTURAL_INVALID_FIXTURES[18][0], "S19", "S19 sits at its frozen index");
  assert.strictEqual(s19.events.length, 2, "S19 holds two events");
  assert.strictEqual(s19.events[0].id, CANONICAL_ID, "S19's first event matches the queried ID");
  assert.strictEqual(
    s19.events[0].receivedAt,
    CANONICAL_TS,
    "S19's first event matches the queried timestamp"
  );
  assert.strictEqual(
    s19.events[1].id,
    s19.events[0].id,
  ...
  const s20 = JSON.parse(STRUCTURAL_INVALID_FIXTURES[19][1]);
  assert.strictEqual(STRUCTURAL_INVALID_FIXTURES[19][0], "S20", "S20 sits at its frozen index");
  assert.strictEqual(s20.events.length, 2, "S20 holds two events");
  assert.strictEqual(s20.events[0].id, CANONICAL_ID, "S20's first event matches the queried ID");
  assert.strictEqual(
    s20.events[0].receivedAt,
    CANONICAL_TS,
    "S20's first event matches the queried timestamp"
  );
  assert.strictEqual(
    Object.hasOwn(s20.events[1], "type"),
    false,
    "S20's second event lacks its own type property"
  );
