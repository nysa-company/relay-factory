...
const TOOL = path.join(__dirname, "..", "tools", "job-event-status-count.js");
const CHILD_TIMEOUT_MS = 5_000;

const USAGE_ERROR =
  "usage: node app/tools/job-event-status-count.js <state-file> <event-id> <status>\n";
const READ_ERROR = "job-event-status-count: cannot read state file\n";
const PARSE_ERROR = "job-event-status-count: state file is not valid JSON\n";
const INVALID_STATE_ERROR = "job-event-status-count: invalid Relay state\n";

const COUNT_TWO_STDOUT = '{"jobsWithEventAndStatus":2}\n';
const COUNT_ONE_STDOUT = '{"jobsWithEventAndStatus":1}\n';
const COUNT_ZERO_STDOUT = '{"jobsWithEventAndStatus":0}\n';

const SENSITIVE_TOKENS = [
  "OPAQUE-EVENT-ID-MUST-NOT-LEAK",
  "EVENT-PAYLOAD-MUST-NOT-LEAK",
  "JOB-ID-TARGET-DEAD-ALPHA-MUST-NOT-LEAK",
  "DEAD-ALPHA-ERROR-MUST-NOT-LEAK",
  "JOB-ID-TARGET-DONE-MUST-NOT-LEAK",
  "IGNORED-JOB-FIELD-MUST-NOT-LEAK",
  "JOB-ID-OTHER-DEAD-MUST-NOT-LEAK",
  "OTHER-DEAD-ERROR-MUST-NOT-LEAK",
  "JOB-ID-TARGET-PENDING-MUST-NOT-LEAK",
  "PENDING-ERROR-MUST-NOT-LEAK",
  "JOB-ID-TARGET-DEAD-BRAVO-MUST-NOT-LEAK",
  ...
  "OTHER-EVENT-ID-MUST-NOT-LEAK",
  "Target-EVENT-ID-MUST-NOT-LEAK",
  " TARGET-EVENT-ID-MUST-NOT-LEAK",
  "ABSENT-EVENT-ID-MUST-NOT-LEAK",
  "pending",
  "done",
  "dead",
  "Dead",
...
const STRUCTURAL_INVALID_FIXTURES = [
  ...
  [
    "S25",
    '{"events":[],"jobs":[{"id":"job-valid-matching-first","eventId":"TARGET-EVENT-ID-MUST-NOT-LEAK","status":"dead","attempts":3,"lastError":"countable-first","retries":0,"attemptsSinceRetry":3},{"id":"job-invalid-missing-attempts","eventId":"event-invalid","status":"pending","lastError":null,"retries":0,"attemptsSinceRetry":0}],"approvals":[],"outbox":[]}\n',
  ],
];

function makeTempDir(label) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `relay-job-event-status-count-${label}-`));
}
...
test("AC1: two invocations for the target event and status dead against Fixture A each print the frozen count of two and leave the fixture untouched", () => {
  ...
  try {
    ...
    for (const attempt of [1, 2]) {
      const result = runTool([fixture, "TARGET-EVENT-ID-MUST-NOT-LEAK", "dead"]);
...
test("AC2: the three single-match conjunction queries against Fixture A each print the frozen count of one and leave the fixture untouched", () => {
  const cases = [
    ["target-done", "TARGET-EVENT-ID-MUST-NOT-LEAK", "done"],
    ["target-pending", "TARGET-EVENT-ID-MUST-NOT-LEAK", "pending"],
    ["other-dead", "OTHER-EVENT-ID-MUST-NOT-LEAK", "dead"],
  ];

  for (const [label, eventId, status] of cases) {
    ...
    try {
      ...
      const result = runTool([fixture, eventId, status]);
...
test("AC3: all four frozen no-match cases succeed with a zero count instead of an error", () => {
  const noMatchEventIds = [
    "Target-EVENT-ID-MUST-NOT-LEAK",
    " TARGET-EVENT-ID-MUST-NOT-LEAK",
    "ABSENT-EVENT-ID-MUST-NOT-LEAK",
  ];
  const storedEventIds = FIXTURE_A_VALUE.jobs.map(job => job.eventId);
  for (const literal of noMatchEventIds) {
    for (const storedEventId of storedEventIds) {
      assert.notStrictEqual(
        literal,
        storedEventId,
        `the direct literal ${JSON.stringify(literal)} is byte-distinct from stored eventId ${JSON.stringify(storedEventId)}`
      );
    }
  }
  ...
  const cases = [
    ["fixture-a-capitalized-event", FIXTURE_A, "Target-EVENT-ID-MUST-NOT-LEAK", "dead"],
    ["fixture-a-leading-space-event", FIXTURE_A, " TARGET-EVENT-ID-MUST-NOT-LEAK", "dead"],
    ["fixture-a-absent-event", FIXTURE_A, "ABSENT-EVENT-ID-MUST-NOT-LEAK", "dead"],
    ["fixture-b-empty-jobs", FIXTURE_B, "TARGET-EVENT-ID-MUST-NOT-LEAK", "dead"],
  ];

  for (const [label, contents, eventId, status] of cases) {
...
test("AC4: every wrong-arity, empty-event-id, and invalid-status case fails with the exact usage envelope before any file access", () => {
  for (const invalidStatus of ["Dead", " dead", ""]) {
    for (const validStatus of ["pending", "done", "dead"]) {
      assert.notStrictEqual(
        invalidStatus,
        validStatus,
        `the direct literal ${JSON.stringify(invalidStatus)} is byte-distinct from valid status ${JSON.stringify(validStatus)}`
      );
    }
  }
  ...
  const cases = [
    ...
    ["two-arguments", candidate => [candidate, "TARGET-EVENT-ID-MUST-NOT-LEAK"]],
    [
      "four-arguments",
      candidate => [candidate, "TARGET-EVENT-ID-MUST-NOT-LEAK", "dead", "extra"],
    ],
    ["empty-event-id", candidate => [candidate, "", "dead"]],
    ["capitalized-status", candidate => [candidate, "TARGET-EVENT-ID-MUST-NOT-LEAK", "Dead"]],
    ["leading-space-status", candidate => [candidate, "TARGET-EVENT-ID-MUST-NOT-LEAK", " dead"]],
    ["empty-status", candidate => [candidate, "TARGET-EVENT-ID-MUST-NOT-LEAK", ""]],
  ];
...
test("AC5: an absent path and a directory path fail with the exact read-failure envelope and change nothing", () => {
  ...
  try {
    ...
    const result = runTool([missing, "TARGET-EVENT-ID-MUST-NOT-LEAK", "dead"]);
  ...
  try {
    ...
    const result = runTool([stateDirectory, "TARGET-EVENT-ID-MUST-NOT-LEAK", "dead"]);
...
test("AC7: every S1-S25 structural-invalid fixture fails with the exact invalid-state envelope after a distinctness proof", () => {
  ...
  assert.strictEqual(s15Status, "DEAD", "S15 freezes the exact status literal DEAD");
  for (const validStatus of ["pending", "done", "dead"]) {
    assert.notStrictEqual(
      s15Status,
      validStatus,
      `S15's DEAD is byte-distinct from valid status ${JSON.stringify(validStatus)}`
    );
  }
  ...
  assert.strictEqual(
    s25Jobs[0].eventId,
    "TARGET-EVENT-ID-MUST-NOT-LEAK",
    "S25's first job matches the queried event ID"
  );
...
test("AC8: the command self-terminates under the frozen timeout and both files stay inside their frozen source boundaries", () => {
  ...
  assert.strictEqual(fs.existsSync(TOOL), true, "app/tools/job-event-status-count.js exists");
