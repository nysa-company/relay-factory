...
function main(args) {
  ...
  try {
    ...
    process.stderr.write("event-metadata: cannot read state file\n");
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write("event-metadata: state file is not valid JSON\n");
    return 1;
  }

  if (!isValidState(state)) {
    process.stderr.write("event-metadata: invalid Relay state\n");
    return 1;
  }

  const event = state.events.find((candidate) => candidate.id === args[1]);
  if (event === undefined) {
    process.stderr.write("event-metadata: event not found\n");
    return 1;
  }
