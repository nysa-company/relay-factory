...
function main(args) {
  ...
  try {
    ...
    process.stderr.write("job-status-error-count: cannot read state file\n");
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write("job-status-error-count: state file is not valid JSON\n");
    return 1;
  }

  if (!isValidState(state)) {
    process.stderr.write("job-status-error-count: invalid Relay state\n");
    return 1;
  }

  const jobsWithStatusAndError = state.jobs.filter(
    (job) =>
      job.status === args[1] &&
      typeof job.lastError === "string" &&
      job.lastError.length > 0,
  ).length;
  process.stdout.write(`${JSON.stringify({ jobsWithStatusAndError })}\n`);
  return 0;
}
