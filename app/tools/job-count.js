...
function main(args) {
  if (args.length !== 1) {
    process.stderr.write("usage: node app/tools/job-count.js <state-file>\n");
    return 2;
  }
  ...
  try {
    ...
    process.stderr.write("job-count: cannot read state file\n");
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write("job-count: state file is not valid JSON\n");
    return 1;
  }

  if (!isValidState(state)) {
    process.stderr.write("job-count: invalid Relay state\n");
    return 1;
  }

  process.stdout.write(`${JSON.stringify({ jobs: state.jobs.length })}\n`);
  return 0;
}
