...
function main(args) {
  ...
  try {
    ...
    process.stderr.write("outbox-approval-count: cannot read state file\n");
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write("outbox-approval-count: state file is not valid JSON\n");
    return 1;
  }

  if (!isValidState(state)) {
    process.stderr.write("outbox-approval-count: invalid Relay state\n");
    return 1;
  }

  const outboxEntries = state.outbox.filter(
    (receipt) => receipt.approvalId === args[1],
  ).length;
