...
function main(args) {
  ...
  try {
    ...
    process.stderr.write("outbox-recipient-count: cannot read state file\n");
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write("outbox-recipient-count: state file is not valid JSON\n");
    return 1;
  }

  if (!isValidState(state)) {
    process.stderr.write("outbox-recipient-count: invalid Relay state\n");
    return 1;
  }

  const outboxEntries = state.outbox.filter((receipt) => receipt.to === args[1]).length;
  process.stdout.write(`${JSON.stringify({ outboxEntries })}\n`);
  return 0;
}
