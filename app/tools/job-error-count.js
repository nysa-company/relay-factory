...
function main(args) {
  if (args.length !== 1) {
    process.stderr.write("usage: node app/tools/job-error-count.js <state-file>\n");
    return 2;
  }
  ...
  try {
    ...
    process.stderr.write("job-error-count: cannot read state file\n");
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write("job-error-count: state file is not valid JSON\n");
    return 1;
  }

  if (!isValidState(state)) {
    process.stderr.write("job-error-count: invalid Relay state\n");
    return 1;
  }

  const jobsWithErrors = state.jobs.filter(
    (job) => typeof job.lastError === "string" && job.lastError.length > 0,
  ).length;
  process.stdout.write(`${JSON.stringify({ jobsWithErrors })}\n`);
  return 0;
}
