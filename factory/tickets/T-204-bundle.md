# T-204 Evidence Bundle — count all Relay jobs offline

## 1. What this does

Relay operators can now count all jobs in a saved state file while Relay is stopped. The command reports only the total count, uses safe errors for invalid input, and does not change the saved file or expose private job or workflow details.

## 2. Preview link

Not applicable — nonvisual PR. [PR #206](https://github.com/nysa-company/relay-factory/pull/206) at exact head `9b53a7d414235cceea1b02808b91ed1465957cec` adds a local offline command with no browser, HTTP, or visual surface. Existing Reviewer approval and every configured required GitHub check passing for this exact protected nonvisual head verify exact job counts, privacy-safe output and errors, complete job validation, read-only operation, and offline termination.

**What to try:** With Relay stopped, run `node app/tools/job-count.js <state-file>` and confirm it prints only the total job count without changing the file.

## 3. Screenshots

Not applicable — nonvisual PR. The exact PR changes only local file-to-output command behavior and has no page, browser view, design reference, or HTTP endpoint to capture. Existing Reviewer approval and passing protected CI at exact head `9b53a7d414235cceea1b02808b91ed1465957cec` verify nonzero and zero counts, fixed redacted errors, complete validation before counting, unchanged files and directories, self-termination, and absence of Relay or network startup.

## 4. Acceptance criteria

| # | Criterion | How it was verified | Result |
|---|---|---|---|
| AC1 | Two runs against the three-job fixture return exactly `{"jobs":3}`, expose no sensitive content, and leave the fixture and directory unchanged. | Named acceptance test `AC1: two invocations against Fixture A each print the frozen three-job count and leave the fixture untouched`; existing Reviewer approval and protected CI passed at the exact PR head. | **PASS** |
| AC2 | The empty-jobs fixture returns exactly `{"jobs":0}`, exposes no opaque sibling content, and changes nothing. | Named acceptance test `AC2: Fixture B prints the frozen zero count while its opaque sibling collections stay unconsumed`; Reviewer approval and protected CI passed. | **PASS** |
...
| AC8 | Complete acceptance tests precede implementation, each stage changes only its owned application file, protected product and Factory surfaces remain unchanged, and all required verification passes. | Reviewer approved the test-first history: test commit `49f1aac` precedes implementation commit `0f30b4e`. Every configured required GitHub check passed at exact head `9b53a7d414235cceea1b02808b91ed1465957cec`, whose complete semantic diff is bound to the protected nonvisual path policy. | **PASS** |
...
**$14.0000 across 7 attempts** from T-204's trusted effective accounting at launch, including this Narrator attempt's conservative reservation.

## 7. Rollback

Revert PR #206 restores the previous behavior.
